...
def obtener_pronostico():
    ...
    weather_url = (
        ...
        "?latitude=" + str(LATITUD) + "&longitude=" + str(LONGITUD) +
        "&hourly=wind_speed_10m,wind_direction_10m"
        "&wind_speed_unit=kmh"
        "&forecast_days=" + str(DIAS_PRONOSTICO) +
...
def encontrar_ventanas_ideales(wind_data, marine_data):
    ...
    marine_idx = {}
    for i, t in enumerate(tiempos_marine):
        marine_idx[t] = i
    ...
    for i, t in enumerate(tiempos_wind):
        ...
        cumple_viento_vel = viento_kmh < VIENTO_MAX_KMH
        cumple_viento_dir = es_direccion_norte(viento_dir)
        cumple_altura = OLA_ALTURA_MIN <= ola_altura <= OLA_ALTURA_MAX
        cumple_periodo = OLA_PERIODO_MIN <= ola_periodo <= OLA_PERIODO_MAX

        if cumple_viento_vel and cumple_viento_dir and cumple_altura and cumple_periodo:
            resultado = {
                "fecha_hora": t,
...
def formatear_mensaje(resultados):
    ...
    lineas = []
    lineas.append("Dias ideales de pesca detectados en Llico")
    lineas.append("")

    for r in resultados:
        fecha = r["fecha_hora"].replace("T", " ")
        linea = fecha + "\n"
        linea = linea + "   Viento: " + str(r["viento_kmh"]) + " km/h (" + str(r["viento_dir"]) + " grados)\n"
        linea = linea + "   Ola: " + str(r["ola_altura"]) + "m / periodo " + str(r["ola_periodo"]) + "s\n"
        lineas.append(linea)
...
def enviar_whatsapp(mensaje):
    if not CALLMEBOT_PHONE or not CALLMEBOT_APIKEY:
        print("CALLMEBOT_PHONE / CALLMEBOT_APIKEY no configurados. No se envia WhatsApp, solo se imprime el resultado.")
        print(mensaje)
        return
    ...
    print("CallMeBot respondio: " + str(resp.status_code) + " - " + resp.text[:200])


def main():
    print("[" + datetime.now(timezone.utc).isoformat() + "] Consultando pronostico...")
    wind_data, marine_data = obtener_pronostico()
    resultados = encontrar_ventanas_ideales(wind_data, marine_data)

    if resultados:
        print("Se encontraron " + str(len(resultados)) + " horas que cumplen los criterios.")
        mensaje = formatear_mensaje(resultados)
        enviar_whatsapp(mensaje)
    else:
        print("Ninguna hora en los proximos " + str(DIAS_PRONOSTICO) + " dias cumple todos los criterios todavia.")
